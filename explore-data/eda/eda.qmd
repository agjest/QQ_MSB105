...
::: {.column width="50%"}
-   the built in pipe is more restrictive
...
`Error in "_"["_" > 0] :    pipe placeholder may only appear once (<input>:4:3) Error during wrapup: not that many frames on the stack Error: no more error handlers available (recursive errors?); invoking 'abort' restart`
:::
:::::
...
## Descriptive Statistics ex. heights
...
## Descriptive Statistics ex. heights {.smaller}
...
heights %>% 
  ...
  vtable::st(
    ...
    as_flextable(     show_coltype = FALSE,     max_row = 30   )
```
:::
